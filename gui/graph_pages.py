...
class GraphPage(Subwindow):
    def __init__(self, field: str, jData: dict):
        ...
        self.df["Timestamp"] = self.df["Timestamp"].dt.round(
            "60min"
        )  # Rounding to the nearest hour makes x-axis auto labelling cleaner
        self.df.set_index("Timestamp", inplace=True)
        logger.info(
            f"Dataset ranges from {self.df.index.min()} to {self.df.index.max()}"
        )
        ...
        self.toolbar = NavigationToolbar2Tk(
            self.canvas, self.master, pack_toolbar=False
        )
        self.toolbar.update()
        ...
        this_week_btn = tk.Button(
            self.master,
            text="This\nWeek",
            command=self.show_this_week,
            bg="#BBBBBB",
            font=fontTuple,
            width=7,
            height=2,
        )
        this_month_btn = tk.Button(
            self.master,
            text="This\nMonth",
            command=self.show_this_month,
            bg="#BBBBBB",
            font=fontTuple,
            width=7,
            height=2,
        )
        this_year_btn = tk.Button(
            self.master,
            text="This\nYear",
            command=self.show_this_year,
            bg="#BBBBBB",
            font=fontTuple,
            width=7,
            height=2,
        ...
        self.canvas.get_tk_widget().pack(
            in_=bottom, side=tk.TOP, fill=tk.BOTH, expand=1
        )
    ...
    def show_previous_week(self):
        ...
        if tentative_new_max <= self.df.index.min():
            logger.info(
                "Cannot go back further -- RHS of plot would be before any data"
            )
        else:
    ...
    def show_previous_month(self):
        ...
        is before the oldest data in our log (i.e. nothing would be plotted).
        """
        tentative_new_max = get_end_of_month(
            self.max_time_to_plot - self.ONE_MONTH
        )  # New right hand side of x-axis
        tentative_new_min = get_start_of_month(
            self.max_time_to_plot - self.ONE_MONTH
        )  # New left hand side of x-axis
    ...
    def show_previous_year(self):
        ...
        is before the oldest data in our log (i.e. nothing would be plotted).
        """
        tentative_new_max = get_end_of_year(
            self.max_time_to_plot - self.ONE_YEAR
        )  # New right hand side of x-axis
        tentative_new_min = get_start_of_year(
            self.max_time_to_plot - self.ONE_YEAR
        )  # New left hand side of x-axis
    ...
    def plot_data(
        self, start_time: datetime.datetime, end_time: datetime.datetime, title=None
    ):
        ...
        try:
            ...
            ErrorPromptPage(
                "Oops! Looks like the telemetry file got corrupted.\nRepair of file attemped...\nPlease retry."
            )
        ...
        self.ax.yaxis.set_major_formatter(
            FormatStrFormatter(self.settings["formatter"])
        )
        ...
        logger.debug(
            f"Values in this window: [{min_val}, {max_val}] --> {yrange} --> {yrange_rounded}"
        )
