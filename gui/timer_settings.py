...
class AquariumLightsSettingsPage(Subwindow):
    ...
    def __init__(self):
        ...
        tk.Label(time_setting_frame, text="Off Time:", font=big_font).grid(
            row=2, column=0
        )
        tk.Label(time_setting_frame, text="Blue at night:", font=big_font).grid(
            row=3, column=0
        )

        self.sunrise_selector = TimeSelector(
            time_setting_frame, self.tank_light_schedule["sunrise_hhmm"]
        )
        self.sunrise_selector.frame.grid(row=1, column=1)

        self.sunset_selector = TimeSelector(
            time_setting_frame, self.tank_light_schedule["sunset_hhmm"]
        )
        self.sunset_selector.frame.grid(row=2, column=1)

        self.blue_at_night_var = tk.StringVar()
        self.blue_at_night_var.set(
            "True" if self.tank_light_schedule["blue_lights_at_night"] else "False"
        )
        OPTIONS = ["True", "False"]
        ...
        self.period_blue_lights_enabled.set(
            "True" if self.tank_light_schedule["eclipse_enabled"] else "False"
        )
        OPTIONS = ["True", "False"]
        w = tk.OptionMenu(
            eclipse_setting_frame, self.period_blue_lights_enabled, *OPTIONS
        )
        w.config(font=big_font)  # set the button font
        menu = eclipse_setting_frame.nametowidget(w.menuname)
        menu.config(font=big_font)  # Set the dropdown menu's font
        ...
        tk.Label(enabled_setting_frame, text="Light1", font=big_font).grid(
            row=1, column=0
        )
        tk.Label(enabled_setting_frame, text="Light2", font=big_font).grid(
            row=2, column=0
        )
        tk.Label(enabled_setting_frame, text="White", font=big_font).grid(
            row=0, column=1
        )
        tk.Label(enabled_setting_frame, text="Blue", font=big_font).grid(
            row=0, column=2
        )
        self.enable_vars = {
            i: {"white_enabled": tk.IntVar(), "blue_enabled": tk.IntVar()}
            for i in [1, 2]
        }

        CHECKBOX_SIZE = 24  # This is ~ the size of the checkmark image /6
        self.on_image = tk.PhotoImage(
            file=os.path.join(ICON_PATH, "checkmark.png")
        ).subsample(6, 6)
        ...
        for l_id in self.enable_vars:
            for c_id, c_var in self.enable_vars[l_id].items():
                ...
                c_var.set(
                    1
                    if self.tank_light_schedule["lights"][f"TankLight{l_id}"][c_id]
                    else 0
                )
    ...
    def save_settings(self):
        ...
        self.tank_light_schedule["blue_lights_at_night"] = (
            True if self.blue_at_night_var.get() == "True" else False
        )

        self.tank_light_schedule["eclipse_enabled"] = (
            True if self.period_blue_lights_enabled.get() == "True" else False
        )
        ...
        for l_id in self.enable_vars:
            for c_id, c_var in self.enable_vars[l_id].items():
                self.tank_light_schedule["lights"][f"TankLight{l_id}"][c_id] = bool(
                    c_var.get()
                )
...
class OutletSettingsPage(Subwindow):
    ...
    def __init__(self):
        ...
        tk.Label(time_setting_frame, textvariable=self.title_var, font=big_font).grid(
            row=0, column=1
        )
        tk.Label(time_setting_frame, text="On Time:", font=big_font).grid(
            row=1, column=0
        ...
        self.optionmenu = tk.OptionMenu(
            time_setting_frame, self.mode_selector, *OPTIONS
        )
        ...
        tk.Label(self.master, textvariable=self.info_text, font=("Arial", 16)).grid(
            row=4, column=0
        )

        tk.Label(
            self.master,
            text="Changes will take effect on next GUI relaunch.",
            font=("Arial", 16),
        ).grid(row=5, column=0)
    ...
    def redraw_for_outlet(self, outlet_name: str):
        ...
        if (this_sched["description"]) != "":
            self.title_var.set(
                f"{outlet_name.capitalize()} ({this_sched['description']})"
            )
        else:
        ...
        self.mode_selector.set(
            this_sched["mode"]
        )  # This will disable the time fields if on/off selected, or otherwise enable them
