...
class MainWindow(Window):
    ...
    def __init__(self, root, fullscreen):
        ...
        self.lightModeText.set(
            "Lights:\n" + self.lightToggleModes[self.currentLightToggleModeInx]
        )

        self.timeText = tk.StringVar()
        self.updateTimestamp()
        tk.Label(root, textvariable=self.timeText, font=("Arial", 25)).place(
            x=425, y=15
        )
        ...
        self.ph_img = tk.PhotoImage(
            file=os.path.join(ICON_PATH, "ph_icon.png")
        ).subsample(3, 3)
        self.temperature_img = tk.PhotoImage(
            file=os.path.join(ICON_PATH, "temperature_icon.png")
        ).subsample(3, 3)

        SCALE = 6
        self.ph_img_small = tk.PhotoImage(
            file=os.path.join(ICON_PATH, "ph_icon.png")
        ).subsample(SCALE, SCALE)
        self.temperature_img_small = tk.PhotoImage(
            file=os.path.join(ICON_PATH, "temperature_icon.png")
        ).subsample(SCALE, SCALE)
        self.settings_img_small = tk.PhotoImage(
            file=os.path.join(ICON_PATH, "settings_icon.png")
        ).subsample(SCALE, SCALE)
        self.fert_img_small = tk.PhotoImage(
            file=os.path.join(ICON_PATH, "peristaltic-pump-2.png")
        ).subsample(9, 9)

        # self.light_img_small = tk.PhotoImage(file=os.path.join(ICON_PATH, "light_icon.png")).subsample(SCALE,SCALE)

        self.light_white = tk.PhotoImage(
            file=os.path.join(ICON_PATH, "light_white.png")
        ).subsample(SCALE, SCALE)
        self.light_blue = tk.PhotoImage(
            file=os.path.join(ICON_PATH, "light_blue.png")
        ).subsample(SCALE, SCALE)
        self.light_timer = tk.PhotoImage(
            file=os.path.join(ICON_PATH, "light_timer.png")
        ).subsample(SCALE, SCALE)
        self.light_off = tk.PhotoImage(
            file=os.path.join(ICON_PATH, "light_off.png")
        ).subsample(SCALE, SCALE)
        ...
        for event in sch_jData["events"]:
            self.the_scheduler.add_event(
                event["name"],
                event["trigger_time_hhmm"],
                lambda: DispensingCapturePage(volume_mL=event["volume_mL"]),
            )

        buttons = [
            {
                "text": self.lightModeText,
                "callback": self.toggle_lights,
                "image": self.light_timer,
            },
            {
                "text": self.temp_value,
                "callback": lambda: GraphPage("Temperature (F)", jData),
                "image": self.temperature_img_small,
            },
            {
                "text": self.ph_value,
                "callback": lambda: GraphPage("pH", jData),
                "image": self.ph_img_small,
            },
            {
                "text": "Peristaltic Pump",
                "callback": lambda: ManualDispensePage(),
                "image": self.fert_img_small,
            },
            {
                "text": "Settings",
                "callback": lambda: SettingsPage(),
                "image": self.settings_img_small,
            },
            {
    ...
    def toggle_lights(self):
        self.currentLightToggleModeInx = (self.currentLightToggleModeInx + 1) % len(
            self.lightToggleModes
        )
        ...
        if newMode == "Schedule":
            ...
            raise ValueError(
                f"Unhandled lightToggleMode [{self.currentLightToggleModeInx}]: {newMode}"
            )
    ...
    def update_ph_for_lock_screen(self, ph: float):
        ...
        self.warning_text.set(prefix + wrap_text(main_window_msg, width=20))
...
class LockScreen(Subwindow):
    ...
    def __init__(self, main_window):
        ...
        self.lock_img = tk.PhotoImage(
            file=os.path.join(ICON_PATH, "lock_icon.png")
        ).subsample(10, 10)
        ...
        tk.Label(
            self.master, textvariable=main_window.timeText, font=("Arial", 30)
        ).place(x=300, y=50)
        tk.Label(frame, image=main_window.ph_img).grid(row=1, column=2, padx=50)
        tk.Label(frame, image=main_window.temperature_img).grid(
            row=1, column=1, padx=50
        )

        tk.Label(frame, textvariable=main_window.temp_value, font=("Arial", 35)).grid(
            row=2, column=1, padx=50
        )
        tk.Label(
            frame, textvariable=main_window.ph_value_for_lock_screen, font=("Arial", 35)
        ).grid(row=2, column=2, padx=50)

        tk.Label(
            self.master,
            textvariable=main_window.lock_screen_ph_text,
            fg="#f00",
        ...
        buttons = [
            {
                ...
                "callback": lambda: ConfirmPromptPage(
                    "Are you sure you want to reboot?", reboot_pi
                ),
            },
            {
                "text": "Aquarium\nLights",
                "callback": lambda: AquariumLightsSettingsPage(),
            },
...
class PeriPumpSettingsPage(Subwindow):
    ...
    def __init__(self):
        ...
        time_setting_frame = tk.LabelFrame(
            self.master, text="Daily Dispense Settings", font=fontTuple
        )

        tk.Label(
            self.master,
            text="Changes will take effect on next GUI restart.",
            font=("Arial", 16),
        ).grid(row=4, column=0)

        time_setting_frame.grid(row=1, column=0, sticky="ew", padx=10, pady=10)

        tk.Label(time_setting_frame, text="Dispense Time:", font=big_font).grid(
            row=1, column=0
        )
        tk.Label(time_setting_frame, text="Volume (mL):", font=big_font).grid(
            row=2, column=0
        )

        self.time_selector = TimeSelector(
            time_setting_frame, self.this_event["trigger_time_hhmm"]
        )
    ...
    def save_settings(self):
        ...
        def hhmmToDatetime(hhmm) -> datetime.datetime:
            ...
            return datetime.datetime.combine(
                datetime.date.today(), datetime.time(hour=hh, minute=mm)
            )
...
class ManualDispensePage(Subwindow):
    def __init__(self):
        ...
        buttons = [
            {
                "text": "Dispense\n1mL",
                "callback": lambda: DispensingCapturePage(1, scheduled=False),
            },
            {
                "text": "Dispense\n3mL",
                "callback": lambda: DispensingCapturePage(3, scheduled=False),
            },
            {
                "text": "Dispense\n10mL",
                "callback": lambda: DispensingCapturePage(10, scheduled=False),
            },
            {
                "text": "Dispense\n15mL",
                "callback": lambda: DispensingCapturePage(15, scheduled=False),
            },
            {
                "text": "Peristaltic Pump\nSettings",
                "callback": lambda: PeriPumpSettingsPage(),
...
class DispensingCapturePage(Subwindow):
    ...
    def __init__(self, volume_mL, scheduled=True):
        super().__init__(
            "Dispense in Progress", draw_exit_button=False, draw_lock_button=False
        )

        buttons = [{"text": "Abort", "callback": self.abort, "color": "#ff5733"}]
        ...
        tk.Label(self.master, text=msg, font=("Arial", 20)).pack(side=tk.TOP, pady=75)
        logger.info("Starting dispense thread")
        self.dispense_stop_event = threading.Event()
        self.dispenseThread = threading.Thread(
            target=dispense,
            args=(hwCntrl, volume_mL, self.dispense_stop_event),
            daemon=True,
        )
...
class CalibratePhStartPage(Subwindow):
    def __init__(self):
        ...
        msg1 = (
            "To calibrate the pH sensor, you will need all three "
            "of the calibration solutions (pH=7.0, 4.0, 10.0)."
        )

        msg2 = "If you've got those ready, go ahead and hit the START button!"
        msg3 = (
        ...
        tk.Label(
            frame,
            text=wrap_text(msg1, 60),
            font=("Arial", 18),
            justify=tk.LEFT,
        ).grid(row=1, column=0, padx=10, pady=10)
        tk.Label(
            frame,
            text=wrap_text(msg2, 60),
            font=("Arial", 18),
            justify=tk.LEFT,
        ).grid(row=2, column=0, padx=10, pady=10)
        tk.Label(
            frame,
            text=wrap_text(msg3, 60),
            font=("Arial", 18),
            fg="red",
            justify=tk.LEFT,
        ...
        btn.grid(row=4, column=0, padx=(400, 20), pady=20)
...
class PhSensorInfoPage(Subwindow):
    def __init__(self):
        super().__init__("pH Sensor Info")

        msg = "To calibrate the pH sensor, you will need all three of the calibration solutions (pH=7.0, 4.0, 10.0).\n"

        calframe = tk.LabelFrame(
            self.master,
        ...
        tk.Label(
            calframe,
            text=f"Next calibration due: {next_cal_due_str}",
            font=("Arial", 15),
            justify=tk.LEFT,
...
class CalibratePhProcessPage(Subwindow):
    def __init__(self):
        ...
        self.titleText.set(
            f"{self.sequence[self.index][2]} calibration @ pH={self.sequence[self.index][0]}"
        )
        tk.Label(
            self.master, textvar=self.titleText, font=("Arial", 22), justify=tk.LEFT
        ).place(x=20, y=20)

        self.line1Text = tk.StringVar()
        self.line1Text.set(
            f"1. Get the {self.sequence[self.index][0]} calibration solution.\n"
        )
        tk.Label(
            self.master, textvar=self.line1Text, font=("Arial", 18), justify=tk.LEFT
        ).place(x=50, y=100)
        ...
        tk.Label(self.master, text=msg2, font=("Arial", 18), justify=tk.LEFT).place(
            x=50, y=125
        )
...
if __name__ == "__main__":
    logger.add(
        os.path.join(os.path.dirname(__file__), "../data/gui.log"),
